...
function Stringifier(scssTree) {
    ...
    if (scssTree._medias.length > 0) {
        for (const m of scssTree._medias) {
            str += '@media(' + m.condition + '){' +
                Parser(m.block, undefined, scssTree).getString() +
                '}';
        }
    }
    return str;
...
function keyframes(parts, ...args) {
    ...
    frames[name] = ['@-webkit-keyframes', '@-moz-keyframes', '@-o-keyframes', '@-ms-keyframes', '@keyframes'].reduce((acc, key) => {
